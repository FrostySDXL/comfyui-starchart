...
class TestNewDoc(unittest.TestCase):
    ...
    def test_mode_path_mismatch_rejected_with_helpful_message(self):
        output = self._tmp_output("mismatch", folder="start-here")
        self._clean(output)
        exit_code, _stdout, stderr = self._run_main(
            "--output",
            output,
        ...
        self.assertNotEqual(exit_code, 0)
        self.assertIn("reference pages usually belong under", stderr)
        self.assertFalse((REPO_ROOT / output).exists())

    def test_mode_path_mismatch_can_be_overridden(self):
        output = self._tmp_output("mismatch_override", folder="start-here")
        self._clean(output)
        exit_code, _stdout, stderr = self._run_main(
            "--output",
            output,
