...
- one simple `STRING`-based node using the legacy V1 class/export pattern still seen in older node packs
...
Use this example as the smallest composition reference. For the retained custom-node guidance around structure, registration, and modern package shape, read `src/content/docs/custom-nodes/node-structure.md`, `src/content/docs/custom-nodes/registration.md`, and `src/content/docs/custom-nodes/development-guide.md` instead of treating this README as a second node reference.
